"""Injected custom CSS: the shared "AI product family" design system.

Light-by-default, premium look (rounded corners, soft shadows, subtle
glass, smooth hover/fade animations) — the same system used by Avia AI and
UzLaw AI, so AI Pharmacy reads as a product from the same company. The
light/dark toggle is done server-side: :func:`inject_custom_css` takes the
current ``dark_mode`` flag from session state and emits the matching color
values directly, re-injecting on every rerun — no client-side JS/
localStorage needed since Streamlit already reruns the whole script on
interaction.
"""
...
def inject_custom_css() -> None:
    ...
    st.markdown(
        ...
        .status-gold {{ background: rgba(245, 158, 11, 0.14); color: #92620A; }}
        .status-indigo {{ background: {theme.PRIMARY_SOFT}; color: {theme.PRIMARY}; }}
